// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using OlaInfrastructure.Data;

#nullable disable

namespace OlaInfrastructure.Migrations
{
    [DbContext(typeof(OlaDbContext))]
    [Migration("20260722232235_ActividadTurnoNullable")]
    partial class ActividadTurnoNullable
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder.HasAnnotation("ProductVersion", "8.0.11");

            modelBuilder.Entity("OlaCore.Models.Actividad", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("AlumnoId")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("TEXT");

                    b.Property<DateTime?>("FechaClase")
                        .HasColumnType("TEXT");

                    b.Property<string>("Tipo")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("TEXT");

                    b.Property<int?>("TurnoId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("AlumnoId");

                    b.HasIndex("Fecha");

                    b.HasIndex("TurnoId");

                    b.ToTable("Actividades");
                });

            modelBuilder.Entity("OlaCore.Models.Alumno", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<bool>("Activo")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Apellido")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT");

                    b.Property<int>("ClasesPendientesRecuperar")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("TEXT");

                    b.Property<DateTime?>("FechaNacimiento")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("FechaRegistro")
                        .HasColumnType("TEXT");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT");

                    b.Property<string>("Notas")
                        .HasColumnType("TEXT");

                    b.Property<bool>("PortalPagosHabilitado")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Telefono")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.ToTable("Alumnos");
                });

            modelBuilder.Entity("OlaCore.Models.Asistencia", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("AlumnoId")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("FechaRegistro")
                        .HasColumnType("TEXT");

                    b.Property<string>("Observaciones")
                        .HasColumnType("TEXT");

                    b.Property<bool>("Presente")
                        .HasColumnType("INTEGER");

                    b.Property<int>("TurnoId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("TurnoId");

                    b.HasIndex("AlumnoId", "TurnoId", "Fecha");

                    b.ToTable("Asistencias");
                });

            modelBuilder.Entity("OlaCore.Models.AusenciaProgramada", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("TEXT");

                    b.Property<int>("InscripcionId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("InscripcionId", "Fecha")
                        .IsUnique();

                    b.ToTable("AusenciasProgramadas");
                });

            modelBuilder.Entity("OlaCore.Models.ConfiguracionSistema", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<string>("Clave")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT");

                    b.Property<string>("Descripcion")
                        .HasColumnType("TEXT");

                    b.Property<string>("Valor")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("Clave")
                        .IsUnique();

                    b.ToTable("ConfiguracionesSistema");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Clave = "HorasAnticipacionCancelacion",
                            Descripcion = "Horas de anticipacion minimas para cancelar una clase",
                            Valor = "24"
                        },
                        new
                        {
                            Id = 2,
                            Clave = "TransferenciaTitular",
                            Descripcion = "Titular de la cuenta para transferencias",
                            Valor = "Yesica Anabela Cruz"
                        },
                        new
                        {
                            Id = 3,
                            Clave = "TransferenciaCVU",
                            Descripcion = "CVU para transferencias",
                            Valor = "0000003100094666029136"
                        },
                        new
                        {
                            Id = 4,
                            Clave = "TransferenciaAlias",
                            Descripcion = "Alias para transferencias",
                            Valor = "ola.ceramica"
                        },
                        new
                        {
                            Id = 5,
                            Clave = "TransferenciaCUIT",
                            Descripcion = "CUIT/CUIL del titular",
                            Valor = "27319388309"
                        },
                        new
                        {
                            Id = 6,
                            Clave = "TransferenciaBanco",
                            Descripcion = "Banco o billetera de destino",
                            Valor = "Mercado Pago"
                        });
                });

            modelBuilder.Entity("OlaCore.Models.DiaSinClase", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("TEXT");

                    b.Property<string>("Motivo")
                        .HasMaxLength(200)
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("Fecha")
                        .IsUnique();

                    b.ToTable("DiasSinClase");
                });

            modelBuilder.Entity("OlaCore.Models.Inscripcion", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<bool>("Activa")
                        .HasColumnType("INTEGER");

                    b.Property<int>("AlumnoId")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("FechaInscripcion")
                        .HasColumnType("TEXT");

                    b.Property<int>("TurnoId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("TurnoId");

                    b.HasIndex("AlumnoId", "TurnoId");

                    b.ToTable("Inscripciones");
                });

            modelBuilder.Entity("OlaCore.Models.Pago", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("AlumnoId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("AnioPago")
                        .HasColumnType("INTEGER");

                    b.Property<int>("CantidadClases")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Comprobante")
                        .HasColumnType("TEXT");

                    b.Property<string>("Detalle")
                        .HasColumnType("TEXT");

                    b.Property<bool>("EsDosVecesSemana")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("FechaPago")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("FechaVencimiento")
                        .HasColumnType("TEXT");

                    b.Property<int>("MesPago")
                        .HasColumnType("INTEGER");

                    b.Property<string>("MetodoPago")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<decimal>("Monto")
                        .HasPrecision(10, 2)
                        .HasColumnType("TEXT");

                    b.Property<decimal>("MontoAbonado")
                        .HasPrecision(10, 2)
                        .HasColumnType("TEXT");

                    b.Property<bool>("MontoAjustadoManual")
                        .HasColumnType("INTEGER");

                    b.Property<decimal>("MontoEsperadoEfectivo")
                        .HasPrecision(10, 2)
                        .HasColumnType("TEXT");

                    b.Property<decimal>("MontoEsperadoTransferencia")
                        .HasPrecision(10, 2)
                        .HasColumnType("TEXT");

                    b.Property<string>("Notas")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("AlumnoId", "MesPago", "AnioPago");

                    b.ToTable("Pagos");
                });

            modelBuilder.Entity("OlaCore.Models.Profesor", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<bool>("Activo")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Apellido")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("TEXT");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT");

                    b.Property<string>("Telefono")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.ToTable("Profesores");
                });

            modelBuilder.Entity("OlaCore.Models.RecuperacionProgramada", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("AlumnoId")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("FechaRegistro")
                        .HasColumnType("TEXT");

                    b.Property<int>("TurnoId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("TurnoId");

                    b.HasIndex("AlumnoId", "TurnoId", "Fecha")
                        .IsUnique();

                    b.ToTable("RecuperacionesProgramadas");
                });

            modelBuilder.Entity("OlaCore.Models.Taller", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<bool>("Activo")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.ToTable("Talleres");
                });

            modelBuilder.Entity("OlaCore.Models.TallerRecuperacionPermitida", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("TallerId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("TallerPermitidoId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("TallerPermitidoId");

                    b.HasIndex("TallerId", "TallerPermitidoId")
                        .IsUnique();

                    b.ToTable("TalleresRecuperacionPermitida");
                });

            modelBuilder.Entity("OlaCore.Models.TarifaMensual", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("Anio")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Mes")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("TallerId")
                        .HasColumnType("INTEGER");

                    b.Property<decimal>("ValorClaseEfectivo")
                        .HasPrecision(10, 2)
                        .HasColumnType("TEXT");

                    b.Property<decimal>("ValorClaseTransferencia")
                        .HasPrecision(10, 2)
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("TallerId");

                    b.HasIndex("Anio", "Mes", "TallerId")
                        .IsUnique();

                    b.ToTable("TarifasMensuales");
                });

            modelBuilder.Entity("OlaCore.Models.Turno", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<bool>("Activo")
                        .HasColumnType("INTEGER");

                    b.Property<int>("CuposMaximos")
                        .HasColumnType("INTEGER");

                    b.Property<int>("DiaSemana")
                        .HasColumnType("INTEGER");

                    b.Property<TimeSpan>("HoraFin")
                        .HasColumnType("TEXT");

                    b.Property<TimeSpan>("HoraInicio")
                        .HasColumnType("TEXT");

                    b.Property<int?>("ProfesorId")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("TallerId")
                        .HasColumnType("INTEGER");

                    b.Property<bool>("UsarFechasManuales")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("ProfesorId");

                    b.HasIndex("TallerId");

                    b.ToTable("Turnos");
                });

            modelBuilder.Entity("OlaCore.Models.TurnoFecha", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("TEXT");

                    b.Property<int>("TurnoId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("TurnoId", "Fecha")
                        .IsUnique();

                    b.ToTable("TurnoFechas");
                });

            modelBuilder.Entity("OlaCore.Models.Usuario", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<bool>("Activo")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("AlumnoId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("TEXT");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int?>("ProfesorId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Rol")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("AlumnoId");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.HasIndex("ProfesorId");

                    b.ToTable("Usuarios");
                });

            modelBuilder.Entity("OlaCore.Models.Actividad", b =>
                {
                    b.HasOne("OlaCore.Models.Alumno", "Alumno")
                        .WithMany()
                        .HasForeignKey("AlumnoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("OlaCore.Models.Turno", "Turno")
                        .WithMany()
                        .HasForeignKey("TurnoId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.Navigation("Alumno");

                    b.Navigation("Turno");
                });

            modelBuilder.Entity("OlaCore.Models.Asistencia", b =>
                {
                    b.HasOne("OlaCore.Models.Alumno", "Alumno")
                        .WithMany("Asistencias")
                        .HasForeignKey("AlumnoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("OlaCore.Models.Turno", "Turno")
                        .WithMany("Asistencias")
                        .HasForeignKey("TurnoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Alumno");

                    b.Navigation("Turno");
                });

            modelBuilder.Entity("OlaCore.Models.AusenciaProgramada", b =>
                {
                    b.HasOne("OlaCore.Models.Inscripcion", "Inscripcion")
                        .WithMany("AusenciasProgramadas")
                        .HasForeignKey("InscripcionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Inscripcion");
                });

            modelBuilder.Entity("OlaCore.Models.Inscripcion", b =>
                {
                    b.HasOne("OlaCore.Models.Alumno", "Alumno")
                        .WithMany("Inscripciones")
                        .HasForeignKey("AlumnoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("OlaCore.Models.Turno", "Turno")
                        .WithMany("Inscripciones")
                        .HasForeignKey("TurnoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Alumno");

                    b.Navigation("Turno");
                });

            modelBuilder.Entity("OlaCore.Models.Pago", b =>
                {
                    b.HasOne("OlaCore.Models.Alumno", "Alumno")
                        .WithMany("Pagos")
                        .HasForeignKey("AlumnoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Alumno");
                });

            modelBuilder.Entity("OlaCore.Models.RecuperacionProgramada", b =>
                {
                    b.HasOne("OlaCore.Models.Alumno", "Alumno")
                        .WithMany()
                        .HasForeignKey("AlumnoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("OlaCore.Models.Turno", "Turno")
                        .WithMany()
                        .HasForeignKey("TurnoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Alumno");

                    b.Navigation("Turno");
                });

            modelBuilder.Entity("OlaCore.Models.TallerRecuperacionPermitida", b =>
                {
                    b.HasOne("OlaCore.Models.Taller", "Taller")
                        .WithMany("RecuperacionesPermitidas")
                        .HasForeignKey("TallerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("OlaCore.Models.Taller", "TallerPermitido")
                        .WithMany()
                        .HasForeignKey("TallerPermitidoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Taller");

                    b.Navigation("TallerPermitido");
                });

            modelBuilder.Entity("OlaCore.Models.TarifaMensual", b =>
                {
                    b.HasOne("OlaCore.Models.Taller", "Taller")
                        .WithMany()
                        .HasForeignKey("TallerId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.Navigation("Taller");
                });

            modelBuilder.Entity("OlaCore.Models.Turno", b =>
                {
                    b.HasOne("OlaCore.Models.Profesor", "Profesor")
                        .WithMany("Turnos")
                        .HasForeignKey("ProfesorId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("OlaCore.Models.Taller", "Taller")
                        .WithMany("Turnos")
                        .HasForeignKey("TallerId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Profesor");

                    b.Navigation("Taller");
                });

            modelBuilder.Entity("OlaCore.Models.TurnoFecha", b =>
                {
                    b.HasOne("OlaCore.Models.Turno", "Turno")
                        .WithMany("FechasManuales")
                        .HasForeignKey("TurnoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Turno");
                });

            modelBuilder.Entity("OlaCore.Models.Usuario", b =>
                {
                    b.HasOne("OlaCore.Models.Alumno", "Alumno")
                        .WithMany()
                        .HasForeignKey("AlumnoId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("OlaCore.Models.Profesor", "Profesor")
                        .WithMany()
                        .HasForeignKey("ProfesorId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Alumno");

                    b.Navigation("Profesor");
                });

            modelBuilder.Entity("OlaCore.Models.Alumno", b =>
                {
                    b.Navigation("Asistencias");

                    b.Navigation("Inscripciones");

                    b.Navigation("Pagos");
                });

            modelBuilder.Entity("OlaCore.Models.Inscripcion", b =>
                {
                    b.Navigation("AusenciasProgramadas");
                });

            modelBuilder.Entity("OlaCore.Models.Profesor", b =>
                {
                    b.Navigation("Turnos");
                });

            modelBuilder.Entity("OlaCore.Models.Taller", b =>
                {
                    b.Navigation("RecuperacionesPermitidas");

                    b.Navigation("Turnos");
                });

            modelBuilder.Entity("OlaCore.Models.Turno", b =>
                {
                    b.Navigation("Asistencias");

                    b.Navigation("FechasManuales");

                    b.Navigation("Inscripciones");
                });
#pragma warning restore 612, 618
        }
    }
}
